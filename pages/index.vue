...
const onClickFavorite = async (item) => {
  ...
  const { data } = await client
    .from("favorites")
    .upsert({ userId: user.value.id, parentId: item.id, isFavorite: true })
    .select("userId, parentId, isFavorite")
    .single();
...
<template>
  <div v-auto-animate>
    ...
    <label for="filterSelect" class="flex text-sm text-gray-700"></label>
    <select
      id="filterSelect"
