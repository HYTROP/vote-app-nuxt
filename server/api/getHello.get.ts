...
export default defineEventHandler(async (event) => {
	...
	const {page, nomination, points} = getQuery(event)
	...
	if (points !== "Все") {
		const {data, error} = await supabase.from("UserPoints").select<"*", any>("*").eq("userID", user!.id)
		if (error) throw error
		if (points == "С оценкой") {
			cursor.filter("photo", "in", `(${data.map((e) => `"${e.cardURL}"`).join(",")})`)
		}
		if (points == "Без оценки") {
			cursor.filter("photo", "not.in", `(${data.map((e) => `"${e.cardURL}"`).join(",")})`)
		}
	}
