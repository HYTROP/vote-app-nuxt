export const useMyFetchItemsStore = defineStore("myFetchItemsStore", () => {
	...
	const fetchFavorites = async () => {
		...
		const {data, error} = await supabase.from("favorites").select("favoritePhotoURLs").eq("userID", user.value.id).single()
	...
	const fetchAllUsersPoints = async () => {
		const {data: allRecordsFromDB} = await supabase.from("myCollection").select("*").order("points", {ascending: true})

		const {data: responseData, error} = await supabase.from("UserPoints").select("*")
		...
		for (const cardURL in cardAverages) {
			...
			const average = pointsArray.reduce((acc, curr) => acc + curr, 0) / pointsArray.length
